...
現在はOpen-Meteoの次の異なる日次集計値を組み合わせている。
...
`precipitation_probability_max` は一日の中で最も高い時間帯の値であるため、晴れを表す日次天気コードと降水確率100%が同時に表示される場合がある。データ取得失敗ではなく、異なる意味の値を同じ時点の予報に見せていることが原因である。

### 修正方針

ルート結果では、札所への到着予定時刻に最も近い時間帯の予報へ統一する。

- Open-Meteoから `hourly=weather_code,temperature_2m,precipitation_probability,relative_humidity_2m` を取得する。
- リクエストには緯度、経度、日付に加えて到着予定時刻を渡す。
- 天気アイコン、気温、降水確率、湿度はすべて同じ時間の配列要素から取得する。
- APIレスポンスへ対象予報時刻 `forecastTime` を含める。
- 画面のtitleまたは補助表示で「到着予定時刻の予報」であることを分かるようにする。
- 到着時刻が取得できない場合だけ日次予報へフォールバックし、その場合の降水確率は「最高降水確率」と明示する。
- キャッシュキーに予報時刻を含める。ただし同一地点・同一時間の重複リクエストはまとめる。

既存の `/weather-proxy` を変更する場合、旧形式の呼び出しを即座に壊さないよう後方互換性を持たせるか、サーバーとフロントを同一コミットで更新する。

### 自動テスト

- 09:00の晴れ・降水確率0%が同じ時間から選ばれる。
- 09:00は晴れ0%、18:00は雨100%の場合、09:00到着では晴れ0%になる。
- 18:00到着では雨100%になる。
- 時刻なしのフォールバックでは「最高降水確率」として扱われる。
- 日付・時刻境界と翌日到着を正しく選択する。

### 完了条件

- 天気、気温、降水確率、湿度が同じ予報時刻に対応する。
- 到着時刻と異なる時間帯の最大降水確率を混在させない。
- 予報対象外の日付では誤った数値を表示しない。
